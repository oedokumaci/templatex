.PHONY: help vscode-settings setup update-dev update-user run pre-commit clean
...
ifeq ($(OS),Windows_NT)
	...
	rmdir /s /q src\templatex\__pycache__ || :
	rmdir /s /q tests\__pycache__ || :
else
	rm -f logs/pytest_test.log
	rm -rf .mypy_cache
	rm -rf .pytest_cache
	rm -rf src/templatex/__pycache__
